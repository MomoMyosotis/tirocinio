...
class statsview_erogation_form extends \moodleform {
    ...
    private function students_indexes($mform) {
        ...
        $erogazione = $DB->get_record('coripodatacollection_erogations', ['id' => $erogationid]);
        ...
        $table->head = [
                ...
                get_string('preliminar_eval', 'mod_coripodatacollection'),
                get_string('gravity_profile', 'mod_coripodatacollection'),
                get_string('evaluation', 'mod_coripodatacollection'),
                get_string('student_report', 'mod_coripodatacollection'),
        ...
        foreach ($indici_valutazione as $ind_val) {
            ...
            if ($ind_val->valutazione_classe == 'Verde')
                continue;
            ...
            $student_in_class = $DB->get_record('coripodatacollection_class_students',
                    ['studentid' => $student->id, 'classid' => $original_res->classe]);
            ...
            $row = [
                    ...
                    compute_preliminar_eval($ind_val, $medie, $stddev),
                    compute_gravity_profile($ind_val)
            ];
        ...
        $this->add_sticky_action_buttons(false,
                get_string('save_evaluation', 'mod_coripodatacollection'));
    ...
    function get_data(): object|array|null {
        ...
        if (!$this->is_cancelled() and $this->is_submitted() and $this->is_validated()) {
            ...
            if (empty($data)) {
                ...
                $display = $this->_customdata['display'];
                if (!isset($data['valutazione_globale']) && $display == 'global')
                    return null;
    ...
    private function get_colored_cell($val, $media, $stddev, $is_partial_index, $outlier, $colname = '', $appendcomputed = false, $inverted = false) {
        ...
        $computed = null;
        // Define which columns are percentages (error-based) and sensible maxes (adjust if needed)
        $percent_max = [
            'lettura_correttezza' => 137, // sum of maximum possible errors (manual estimation)
            'scrittura_correttezza' => 107,
            'matematica_correttezza_lettura' => 20,
            'matematica_correttezza_enumerazione' => 20,
            'matematica_correttezza_decodifica' => 40,
            'matematica_correttezza_calcolo' => 20
        ];
